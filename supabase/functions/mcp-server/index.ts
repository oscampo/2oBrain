...
mcp.tool('search', {
  description:
    'Busca hechos vigentes en el segundo cerebro por una pregunta en lenguaje natural. Devuelve los hechos más relevantes, con su fuente.',
  ...
  handler: async ({ query }: { query: string }) => {
    ...
    // para el/los nodo(s) nombrados), seguidos de los de la búsqueda
...
mcp.tool('remember', {
  description:
    'Registra un hecho atómico con fecha y fuente obligatorias en el segundo cerebro. Antes de insertar, busca hechos vigentes parecidos por embedding; si encuentra candidatos, se niega a insertar salvo que se pase supersedes o distinct explícito. El nodo (o nodos) debe existir de antemano en la tabla nodes salvo que se pase createNode.',
  inputSchema: z.object({
    ...
    source: z.string().describe('De dónde salió el hecho'),
    ...
    createNode: z.boolean().optional().describe('Crea el/los nodo(s) si no existen todavía, en vez de fallar'),
    ...
    confirmDate: z.boolean().optional().describe('Obligatorio si date no es la fecha real de hoy (America/Bogota) -- confirma que un hecho con fecha distinta es intencional (histórico, backfill), no un error de no verificar la fecha antes de llamar'),
  }),
  handler: async (args: {
    claim: string;
    date: string;
    ...
    if (args.date !== todayBogota && !args.confirmDate) {
      return {
        content: [{
          type: 'text',
          text: `date ${args.date} es distinto de hoy (${todayBogota} en America/Bogota). Si es un hecho histórico o backfill intencional, pasa confirmDate: true. Si fue sin querer, corrige date y vuelve a intentar.`,
        }],
        isError: true,
      };
    }
    ...
    if (similar.length > 0 && supersedesIds.length === 0 && !distinct) {
      let text = `Hay ${similar.length} hecho(s) vivo(s) parecido(s), resuélvelo antes de insertar:\n`;
