...
| brain-hygiene | weekly-equivalent | no | `node scripts/db/doctor.mjs --fix` (`--fix` applies the mechanical checks -- RLS, superseded_by, memory_links/memory_pair_checks orphaned by a merge -- without asking; the rest still needs a human decision and stays a plain WARN); relay anything still WARN. |
...
| commitments-check | first session of the day | no | `node scripts/db/list-commitments.mjs` (query against `records`, not `MEMORY.md` prose); surface anything due or overdue. Backup pass (2026-09-08): for each open one, skim newer records in the same recuerdo(s) for signs it's already (partially) resolved -- the primary defense is the write-time check documented in `skills/segundo-cerebro-capture/SKILL.md`, this only catches what slipped past that. |
| memory-prune | weekly-equivalent | no | The `MEMORY.md` maintenance ritual (promote / demote / cut). |
| morning-briefing | first session of the day | no | One screen: due today, waiting on, worth knowing. No filler, a skipped briefing costs less than an empty one. If you've connected mail/calendar tools, use them to complement it; if not, skip that part rather than guessing. If you do have candidate items from mail/calendar, cross-referencing them against your own records is a **mechanical step, not optional**: list each one as `label \| search text`, pipe them through `node scripts/db/briefing-crossref.mjs` in ONE call, and compose the briefing text FROM that output (which record already covers it, or "SIN COINCIDENCIA" for genuinely new) -- not from memory of what's tracked. A prose reminder to "cross-reference before reporting" is easy to skip under pressure; a script whose output is the only source of the text is not. |
