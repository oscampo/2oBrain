...
// vez de la terminal. Decisión y alcance: registro #251/#252.
//
...
// Lista de recuerdos vigentes para poblar el autocompletado de Timeline y
// Guardar registro: mismo motivo que /api/pages, pero para record_memories.
app.get('/api/memories', (c) => {
  const result = runScript('list-memories.mjs', ['--json']);
  if (!result.ok) return c.json({ ok: false, error: result.stderr || 'list-memories.mjs falló' }, 422);
  try {
    return c.json({ ok: true, ...JSON.parse(result.stdout) });
  } catch {
    return c.json({ ok: false, error: 'list-memories.mjs no devolvió JSON válido' }, 422);
  }
});
...
// llamada consulta el estado vigente de memories/record_memories/memory_links, así
// que abrir/refrescar la sección "Grafo" siempre refleja el último registro o
// recuerdo integrado. El layout (posiciones, física, drag/zoom) corre en el
...
app.get('/api/memory-status', (c) => {
  ...
  if (!node) return c.json({ ok: false, error: 'falta ?memory=' }, 400);
  const args = [node];
  if (provider) args.push('--provider', provider);
  if (model) args.push('--model', model);
  return respond(c, runScript('memory-status.mjs', args));
});
...
// imprimía "[i/n] claim" por registro a stderr según avanzaba, pero
...
// config/ollama-models.json) que usan extract-records.mjs y
// extract-page-records.mjs: un único origen de verdad, editable a mano o
...
app.post('/api/merge-memories', async (c) => {
  const body = await c.req.json().catch(() => null);
  if (!body?.from || !body?.to || !body?.reason) {
    return c.json({ ok: false, error: 'faltan from/to/reason' }, 400);
  }
  return respond(c, runScript('merge-memories.mjs', ['--from', body.from, '--to', body.to, '--reason', body.reason]));
});
...
app.get('/api/category-candidates', (c) => {
  ...
  try {
    ...
    return c.json({ ok: false, error: 'list-category-candidates.mjs no devolvió JSON válido: ' + result.stdout.slice(0, 500) }, 422);
  }
});
...
app.get('/api/mention-candidates', (c) => respond(c, runScript('list-memory-mentions.mjs', [])));

app.get('/api/memory-links', (c) => {
  const node = c.req.query('memory');
  if (!node) return c.json({ ok: false, error: 'falta ?memory=' }, 400);
  return respond(c, runScript('memory-link.mjs', ['--list', node]));
});

app.post('/api/memory-link', async (c) => {
  const body = await c.req.json().catch(() => null);
  ...
  return respond(c, runScript('memory-link.mjs', ['--from', body.from, '--to', body.to, '--relation', body.relation, '--date', body.date, '--reason', body.reason]));
});

app.post('/api/memory-unlink', async (c) => {
  const body = await c.req.json().catch(() => null);
  if (!body?.from || !body?.to || !body?.reason) {
    return c.json({ ok: false, error: 'faltan from/to/reason' }, 400);
  }
  ...
  return respond(c, runScript('memory-unlink.mjs', args));
});
...
// mismo patrón "revisión humana obligatoria" que Fusionar recuerdos.
app.get('/api/alias-candidates', (c) => {
  const node = c.req.query('memory');
  return respond(c, runScript('list-alias-candidates.mjs', node ? [node] : []));
});

app.post('/api/set-memory-aliases', async (c) => {
  const body = await c.req.json().catch(() => null);
  if (!body?.memory || !body?.reason || (!body?.aliases && !body?.remove)) {
    return c.json({ ok: false, error: 'faltan memory/reason y (aliases o remove)' }, 400);
  }
  const args = ['--memory', body.memory, '--reason', body.reason];
  args.push(body.remove ? '--remove' : '--aliases', body.remove || body.aliases);
  return respond(c, runScript('set-memory-aliases.mjs', args));
});

// Reasigna UN registro a otro recuerdo sin tocar el resto del recuerdo (distinto de
// Fusionar recuerdos, que mueve todos los registros): usado cuando
// classify-memory.mjs se equivocó o el registro se guardó con --memory a mano sin
...
// extract-page-records.mjs sí tiene endpoint (2026-08-31), a diferencia de
// extract-records.mjs de arriba: --json no pregunta nada por stdin (a
// diferencia de --review), entrega los candidatos con sus recuerdos parecidos
...
app.get('/api/extract-page-records', (c) => {
  ...
  if (provider) args.push('--provider', provider);
  if (model) args.push('--model', model);
  const result = runScript('extract-page-records.mjs', args);
  if (!result.ok) return c.json({ ok: false, error: result.stderr || 'extract-page-records.mjs falló' }, 422);
  try {
    return c.json({ ok: true, ...JSON.parse(result.stdout) });
  } catch {
    return c.json({ ok: false, error: 'extract-page-records.mjs no devolvió JSON válido: ' + result.stdout.slice(0, 500) }, 422);
  }
});
